...
| Domain | APIs | Operations | Description |
|---|---|---|---|
| Clients & Groups | 4 | 21 | Client and group management, documents, comments |
| Loans | 9 | 76 | Loan accounts, transactions, schedule, tranches, products, downloads |
| Deposits | 5 | 57 | Deposit accounts, transactions, balance summary, products, downloads |
| Accounting & GL | 4 | 8 | Interest accrual, reports, GL accounts, journal entries |
| Cards & Authorization | 1 | 11 | Card transactions and authorization holds |
| Credit Arrangements | 1 | 12 | Credit arrangement lifecycle and accounts |
| Configuration | 17 | 37 | System configuration for branches, currencies, products, roles, etc. |
| Organization | 7 | 12 | Holidays, transaction channels, ID templates, setup |
| Currencies & Rates | 4 | 16 | Currencies, exchange rates, accounting rates, index rates |
| Users & Security | 4 | 23 | Users, roles, API consumers, key rotation |
| Profit Sharing | 5 | 18 | Pools, cash flows, products, proposals, settings |
| System & Operations | 23 | 87 | Background processes, branches, communications, crons, data import, documents, subscriptions, tasks, templates, and more |

<details>
<summary><strong>Clients & Groups</strong> (4 APIs, 21 operations)</summary>
...
<details>
<summary><strong>Loans</strong> (9 APIs, 76 operations)</summary>
...
| `loans/list_authorization_holds` | Get authorization holds related to a loan account |
...
| `loans/create_card` | Create and associate a new card to an account |
| `loans/delete_card` | Delete a card associated to an account |
...
| `loans/apply_planned_fees` | Apply planned fees from past or future installments |
...
| `loans/change_periodic_payment` | Change the periodic payment amount for an active loan |
...
| `loans/get_loan_account_rsv` | Get repayment schedule versioning for a loan account |
...
| `loans_transactions/make_principal_overpayment` | Make non-scheduled principal overpayment transaction |
...
| `loans_transactions/apply_unlock` | Unlock loan account income sources |
| `loans_transactions/make_withdrawal` | Make withdrawal from redraw balance |
| `loans_transactions/make_deposit_on_credit_balance` | Create a CREDIT_BALANCE_DEPOSIT transaction |

#### loans_transactions_octetstream
...
| Label | Description |
|---|---|
| `funding_sources/sell` | Sell a funding share owned by an investor |

</details>

<details>
<summary><strong>Deposits</strong> (5 APIs, 57 operations)</summary>
...
| `deposits/list_authorization_holds` | Get authorization holds related to a deposit account |
| `deposits/create_authorization_hold` | Create an authorization hold for an account |
...
| `deposits/unblock_fund` | Unblock a previously blocked fund |
| `deposits/patch_block_fund` | Update the amount of an existing blocked fund |
| `deposits/list_cards` | Get cards associated with an account |
| `deposits/create_card` | Create and associate a new card to an account |
| `deposits/delete_card` | Delete a card associated to an account |
| `deposits/get_funded_loans` | Get all loan accounts funded by the deposit account |
| `deposits/get_schedule_for_funded_account` | Get loan account schedule for a funded account |
...
| `deposits/apply_interest` | Apply accrued interest |
| `deposits/change_interest_rate` | Change deposit account interest rate |
| `deposits/change_state` | Change deposit account state |
| `deposits/change_withholding_tax` | Change deposit account withholding tax rate |
| `deposits/reopen` | Reopen a deposit account |
| `deposits/start_maturity` | Start maturity for deposit account |
| `deposits/transfer_ownership` | Transfer account ownership to a new holder |
| `deposits/undo_maturity` | Undo the maturity period for a deposit account |
...
| `deposits_transactions/adjust` | Adjust a deposit transaction |
| `deposits_transactions/search` | Search deposit transactions |
...
| `deposits_transactions/make_deposit_async` | Create deposit transaction (async) |
| `deposits_transactions/make_withdrawal_async` | Create withdrawal transaction (async) |
...
<details>
<summary><strong>Accounting & GL</strong> (4 APIs, 8 operations)</summary>

#### accounting_interest_accrual

| Label | Description |
|---|---|
| `accounting_interest_accrual/search` | Search interest accrual breakdown entries |
...
<details>
<summary><strong>Cards & Authorization</strong> (1 API, 11 operations)</summary>

#### cards

| Label | Description |
|---|---|
| `cards/create_authorization_hold` | Create an authorization hold for a card |
...
| `cards/decrease_authorization_hold` | Decrease the amount of an authorization hold |
| `cards/increase_authorization_hold` | Increase authorization hold amount |
| `cards/create_bulk_authorization_holds` | Create bulk authorization holds for a card |
| `cards/get_account_balances` | Get account balances using card tokens |
| `cards/create_card_transaction` | Create a financial transaction for a card |
...
<details>
<summary><strong>Credit Arrangements</strong> (1 API, 12 operations)</summary>
...
<details>
<summary><strong>Configuration</strong> (17 APIs, 37 operations)</summary>

#### configuration_accounting_rules_yaml
...
#### configuration_authorization_holds_yaml
...
#### configuration_client_roles_yaml
...
| `configuration_custom_fields/get_template` | Get custom field definitions configuration template |

#### configuration_deposit_products_yaml
...
#### configuration_end_of_day_processing_yaml
...
#### configuration_group_role_names_yaml
...
#### configuration_id_document_templates_yaml
...
#### configuration_index_rates_yaml
...
#### configuration_internal_controls_yaml
...
#### configuration_loan_products_yaml

| Label | Description |
|---|---|
| `configuration_loan_products_yaml/get` | Get loan products configuration |
| `configuration_loan_products_yaml/update` | Update loan products configuration |

#### configuration_loan_risk_levels_yaml
...
#### configuration_transaction_channels_yaml
...
<details>
<summary><strong>Organization</strong> (7 APIs, 12 operations)</summary>
...
#### organization_holidays_nonworkingdays
...
#### organization_identification_document_templates

| Label | Description |
|---|---|
| `organization_identification_document_templates/list` | Get ID templates |

#### organization_transaction_channels
...
<details>
<summary><strong>Currencies & Rates</strong> (4 APIs, 16 operations)</summary>
...
<details>
<summary><strong>Users & Security</strong> (4 APIs, 23 operations)</summary>
...
<details>
<summary><strong>Profit Sharing</strong> (5 APIs, 18 operations)</summary>

#### profit_sharing_cashflows

| Label | Description |
|---|---|
| `profit_sharing_cashflows/get_cash_flows` | Get a list of cash flows |
| `profit_sharing_cashflows/create_cash_flow` | Create a new cash flow |
| `profit_sharing_cashflows/get_cash_flow_by_id` | Get a single cash flow |
...
| `profit_sharing_cashflows/get_cash_flow_settings_by_id` | Get settings for a cash flow |
| `profit_sharing_cashflows/update_cash_flow_settings` | Update settings for an existing cash flow |

#### profit_sharing_pools

| Label | Description |
|---|---|
| `profit_sharing_pools/get_pools` | Get a list of pools |
| `profit_sharing_pools/create_pool` | Create a new investment pool |
| `profit_sharing_pools/get_pool_by_id` | Get a single pool |
...
| `profit_sharing_pools/get_pool_settings_by_id` | Get settings for an investment pool |
| `profit_sharing_pools/update_pool_settings` | Update settings for an existing investment pool |

#### profit_sharing_product_settings

| Label | Description |
|---|---|
| `profit_sharing_product-settings/create_product_settings` | Create new product settings |
| `profit_sharing_product-settings/get_product_settings_by_id` | Get product settings |
| `profit_sharing_product-settings/update_product_settings` | Update existing product settings |
| `profit_sharing_product-settings/search_product_settings` | Search product settings by criteria |

#### profit_sharing_products

| Label | Description |
|---|---|
| `profit_sharing_products/list` | Get Islamic savings products |

#### profit_sharing_proposals

| Label | Description |
|---|---|
| `profit_sharing_proposals/find_proposal_account_details` | Get proposal account details and payment/profit cycle calculation |
| `profit_sharing_proposals/find_proposals` | Get proposals calculation |
| `profit_sharing_proposals/approve_proposal` | Approve a proposal |

</details>

<details>
<summary><strong>System & Operations</strong> (23 APIs, 87 operations)</summary>

#### application_status

| Label | Description |
|---|---|
| `application_status/get` | Retrieve the state of application data access |

#### archive_deposits_transactions_custom_fields
...
| Label | Description |
|---|---|
| `background_process/get_latest_by_type` | Get the latest EOD process by type |
| `background_process/update` | Cancel EOD processes using the encoded key |
...
| Label | Description |
|---|---|
| `bulks/get_bulk_status` | Get the status of a bulk process via key |
...
| `communications_messages/enqueue_by_date` | Resend failed messages asynchronously by date |
| `communications_messages/enqueue_by_keys` | Resend failed messages asynchronously using keys |
| `communications_messages/search` | Search communication messages |
| `communications_messages/search_sorted` | Search sorted communication messages |

#### crons_early_eod

| Label | Description |
|---|---|
| `crons_early_eod/run_earlier_hourly_and_end_of_day_crons` | Trigger hourly and EOD processing earlier, on the current day |

#### crons_eod

| Label | Description |
|---|---|
| `crons_eod/run_hourly_and_end_of_day_crons` | Trigger hourly and EOD processing on the previous day |
...
| `data_import/action` | Approve or reject a data import event |
| `data_import/get_import` | Retrieve a data import response |
...
| Label | Description |
|---|---|
| `subscriptions/create_or_get` | Create a streaming events subscription |
| `subscriptions/delete` | Delete a streaming events subscription |
